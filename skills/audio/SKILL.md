...
description: >
  ...
  write. Falls back gracefully when API key is not configured.
...
- If not set: guide the user:
  "Audio generation requires a Google AI API key. Get one free at https://aistudio.google.com/apikey
   ...
   This is the same key used by `/smart-blog-skills:image` -- if image generation works, audio works too."
- **When called internally** (from write): return silently if key is missing.
...
2. If image is configured, the key is already available
