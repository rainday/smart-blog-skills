...
description: >
  ...
  currency, and quote formatting. Invoke from `translate` and
  `multilingual` orchestrators when a single source-to-target language
...
- `translate` (one agent per target language, run in parallel).
- `multilingual` (delegated through `translate`).
...
- Apply locale-specific number, date, currency, and quote formats.
